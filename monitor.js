...
function appendCSV(rows) {
  ...
  if (!fs.existsSync(CSV_FILE)) fs.writeFileSync(CSV_FILE, 'timestamp,pagina,dispositivo,score,lcp_s,fcp_s,tbt_ms,cls,speedindex_s,caida\n');
  fs.appendFileSync(CSV_FILE, rows.map(r => r.join(',')).join('\n') + '\n');
}

// --- Evaluación: ¿está mal este resultado? ----------------------------------
function evaluate(res, ff) {
  if (res.down) return { bad: true, detail: 'no responde / error de carga' };
  const t = THRESHOLDS[ff];
...
function preview() {
  ...
  const ok = (score, lcp) => ({ down: false, score, lcp, fcp: +(lcp * 0.7).toFixed(2), tbt: 0, cls: 0, si: lcp, runs: 2 });
  const goodByPage = {
