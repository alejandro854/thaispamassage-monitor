...
// Agrega el log de masajes por URL desde una fecha: veces escaneada + medias móvil/escritorio.
function aggregateMassages(log, sinceMs) {
  const byU = {};
  for (const e of (log || [])) {
    if (Date.parse(e.ts) < sinceMs) continue;
    const u = byU[e.url] = byU[e.url] || { url: e.url, category: e.category, scans: 0, mob: { n: 0, s: 0, l: 0 }, desk: { n: 0, s: 0, l: 0 } };
  ...
  return Object.values(byU).map(u => ({
...
export function updateSummary(byPage, { alerting = [], events = [], ts, targets = [] }) {
  ...
  // Agregado por URL (7 días) que consume también el panel de WordPress.
  s.massages = aggregateMassages(s.massageLog, Date.parse(ts) - 7 * 864e5);
...
export function weeklyStats() {
  ...
  const massages = aggregateMassages(s.massageLog, Date.now() - 7 * 864e5);
