...
// Logo PNG (los emails no renderizan SVG; este PNG tiene fondo transparente y va bien sobre oscuro)
const LOGO = 'https://thaispamassage.es/wp-content/uploads/2022/06/logo-thaispamassage.png';
...
export function renderEmail({ type, byPage, timestamp, alerts = [] }) {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${C.paper};padding:24px 12px;"><tr><td align="center">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:16px;overflow:hidden;border:1px solid ${C.line};">
      <!-- Cabecera -->
      <tr><td style="background:${C.ink};padding:30px 24px 26px;border-bottom:4px solid ${accent};text-align:center;">
        <img src="${LOGO}" alt="${SITE_NAME}" width="72" height="64" style="height:64px;width:auto;display:inline-block;border:0;">
        <div style="color:${C.gold};font-size:12px;letter-spacing:.14em;text-transform:uppercase;margin-top:16px;">Monitor de velocidad</div>
        <div style="margin-top:16px;">${statusPill}</div>
      </td></tr>
      ${alertBox}
      <!-- Tabla de resultados -->
...
export function subjectFor(type, alerts) {
  if (type === 'alert') {
    ...
    return `${SITE_NAME} — Aviso de rendimiento: ${who}`;
  }
  if (type === 'recovery') return `${SITE_NAME} — Rendimiento recuperado`;
  if (type === 'digest') return `${SITE_NAME} — Resumen semanal de velocidad`;
  return `${SITE_NAME} — Informe de velocidad`;
}

// --- Email de resumen semanal (viernes) ------------------------------------
export function renderDigest({ perDay = [], uptime = 100, alerts = [], timestamp, massages = [] }) {
  ...
  const incBox = alerts.length ? `
    <tr><td style="padding:6px 24px 0;">
      ...
      ${alerts.map(a => `<div style="font-size:13px;color:${C.ink};margin:3px 0;"><span style="color:${a.type === 'recovery' ? C.green : C.red};font-weight:800;">&#9679;</span> <b>${a.page}</b> (${a.profile}) — ${a.detail} <span style="color:${C.gray};">· ${new Date(a.ts).toLocaleString('es-ES', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</span></div>`).join('')}
    </td></tr>` : `
    <tr><td style="padding:12px 24px 0;"><div style="padding:12px 14px;background:${C.greenBg};border-radius:10px;color:${C.green};font-weight:700;font-size:13px;text-align:center;">Sin incidencias esta semana. La web ha funcionado correctamente.</div></td></tr>`;

  // --- Tabla de fichas de masaje (rotación por categoría): URL, veces y medias ---
  const shortUrl = u => (u.replace(/^https?:\/\/[^/]+/, '').replace(/\/$/, '') || '/');
  const mrows = massages.map(m => `<tr>
    <td style="padding:8px 12px;border-top:1px solid ${C.line};font-size:12px;color:${C.gray};">${m.category}</td>
    <td style="padding:8px 12px;border-top:1px solid ${C.line};font-size:12px;color:${C.ink};">${shortUrl(m.url)}</td>
    <td style="padding:8px 12px;border-top:1px solid ${C.line};text-align:center;font-size:14px;color:${C.ink};font-weight:800;">${m.scans}</td>
    <td style="padding:8px 12px;border-top:1px solid ${C.line};text-align:center;font-size:12px;color:${C.gray};">${m.mob ? m.mob.score + ' · ' + m.mob.lcp + 's' : '—'}</td>
    <td style="padding:8px 12px;border-top:1px solid ${C.line};text-align:center;font-size:12px;color:${C.gray};">${m.desk ? m.desk.score + ' · ' + m.desk.lcp + 's' : '—'}</td>
  </tr>`).join('');
  const massBox = massages.length ? `
    <tr><td style="padding:18px 24px 4px;">
      <div style="font-size:12px;color:${C.gray};text-transform:uppercase;letter-spacing:.05em;margin:8px 0 8px;">Fichas de masaje · rotación por categoría (1 al azar de cada una por medición)</div>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
        <tr>
          <th style="text-align:left;padding:0 12px 8px;font-size:11px;color:${C.gray};text-transform:uppercase;">Categoría</th>
          <th style="text-align:left;padding:0 12px 8px;font-size:11px;color:${C.gray};text-transform:uppercase;">Página</th>
          <th style="text-align:center;padding:0 12px 8px;font-size:11px;color:${C.gray};text-transform:uppercase;">Veces</th>
          <th style="text-align:center;padding:0 12px 8px;font-size:11px;color:${C.gray};text-transform:uppercase;">Móvil</th>
          <th style="text-align:center;padding:0 12px 8px;font-size:11px;color:${C.gray};text-transform:uppercase;">Escrit.</th>
        </tr>
        ${mrows}
      </table>
      <div style="font-size:10px;color:${C.gray};padding:6px 12px 0;">Móvil / Escritorio = media de puntuación · LCP (segundos) de cada URL en los scaneos de la semana.</div>
    </td></tr>` : '';

  return `<!doctype html><html><body style="margin:0;background:${C.paper};font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${C.paper};padding:24px 12px;"><tr><td align="center">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:16px;overflow:hidden;border:1px solid ${C.line};">
      <tr><td style="background:${C.ink};padding:30px 24px 26px;border-bottom:4px solid ${C.gold};text-align:center;">
        <img src="${LOGO}" alt="${SITE_NAME}" width="72" height="64" style="height:64px;width:auto;display:inline-block;border:0;">
        <div style="color:${C.gold};font-size:12px;letter-spacing:.14em;text-transform:uppercase;margin-top:16px;">Resumen semanal de velocidad</div>
      </td></tr>
      <tr><td style="padding:18px 24px 4px;">
