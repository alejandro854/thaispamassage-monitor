...
function lcpColor(lcp, ff) {
  const poor = THRESHOLDS[ff].lcp, good = ff === 'mobile' ? 2.5 : 1.5;
  return lcp == null ? C.gray : lcp <= good ? C.green : lcp < poor ? C.amber : C.red;
}
...
function metricCell(m, ff) {
  ...
  const sc = scoreColor(m.score), lc = lcpColor(m.lcp, ff);
  return `<td style="padding:10px 12px;border-top:1px solid ${C.line};text-align:center;white-space:nowrap;">
    ...
    <div style="font-size:12px;color:${lc};margin-top:5px;font-weight:600;">LCP ${m.lcp != null ? m.lcp + 's' : '—'}</div>
  </td>`;
}
...
export function renderEmail({ type, byPage, timestamp, alerts = [] }) {
  ...
  const rows = PAGES.map((pg) => {
    ...
    return `<tr>
      ...
      ${metricCell(mob, 'mobile')}
      ${metricCell(dsk, 'desktop')}
    </tr>`;
  }).join('');
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${C.paper};padding:24px 12px;"><tr><td align="center">
    ...
    <div style="font-size:11px;color:${C.gray};margin-top:14px;">Monitor de velocidad — AJ Dev</div>
  </td></tr></table>
  </body></html>`;
}
